...
def plot(idx, rewards, losses):
    ...
    plt.subplot(131)
    plt.title('frame %s. reward: %s' % (idx, np.mean(rewards[-20:])))
    plt.plot(rewards)
    plt.subplot(132)
    plt.title('loss')
    plt.plot(losses)
...
replay_buffer = ReplayBuffer(size=60000)
batch_size = 64
gamma = 0.95
...
def compute_loss(current_model, target_model):
    s, a, r, n_s, d = replay_buffer.sample(batch_size)
    ...
    next_q_values = current_model(n_s)
    next_q_state_values = target_model(n_s)
...
def update_target(curr, target):
    target.load_state_dict(curr.state_dict())
...
if __name__ == '__main__':
    ...
    curr_model = Model(state_size, action_size).to(device)
    tar_model = Model(state_size, action_size).to(device)
    update_target(curr_model, tar_model)

    optimizer = optim.Adam(params=curr_model.parameters(), lr=1e-05)
    ...
    for frame_idx in range(1, num_frames):
        action = curr_model.action(state, epsilon_by_frame(frame_idx))
        ...
        if len(replay_buffer) > replay_initial:
            loss = compute_loss(curr_model, tar_model).item()
            losses.append(loss)
